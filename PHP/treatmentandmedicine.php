...
else
{
    if (!empty($date) || !empty($matricnumber) || !empty($name) || !empty($diagnosis_system) || !empty($clinical_finding) || !empty($symptom) || !empty($investigation) || !empty($treatment) || !empty($name_of_medicine) || !empty($dos) || !empty($frequency)|| !empty($duration))
    {
        ...
        {
            $SELECT = "SELECT Matric_Num FROM treatment_and_medicine WHERE Matric_Num = ? LIMIT 1";
              
            $INSERT = "INSERT INTO treatment_and_medicine(Date, Matric_Num, Name, Diagnosis_system, Clinical_finding, Symptom, Investigation, Treatment, Name_of_medicine, Dos, Frequency, Duration) values(?,?,?,?,?,?,?,?,?,?,?,?)";
